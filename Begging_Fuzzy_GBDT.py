...
# If the transformed feature is categorical, for example Low/Medium/High,
...
# 6. Define Bagging GBDT model

# estimator is used in newer scikit-learn versions.
# The fallback keeps the code usable in older versions.

try:
    bagging_gbdt_model = BaggingClassifier(
        estimator=base_gbdt,
        n_estimators=10,
        max_samples=0.8,
        max_features=1.0,
...
bagging_gbdt_model.fit(X_train, y_train)
...
y_pred = bagging_gbdt_model.predict(X_test)

# Probability of positive class, needed for ROC-AUC
y_prob = bagging_gbdt_model.predict_proba(X_test)[:, 1]



# 9. Performance metrics on test data
...
print("\n====================================")
print("Bagging GBDT Test Results")
print("====================================")
...
roc_path = os.path.join(
    figure_folder,
    "bagging_gbdt_transformed_feature_roc_auc.png"
)

plt.figure(figsize=(8, 6))
plt.plot(fpr, tpr, label=f"Bagging GBDT, AUC = {auc:.4f}")
plt.plot([0, 1], [0, 1], linestyle="--", label="Random classifier")

plt.xlabel("False Positive Rate")
plt.ylabel("True Positive Rate")
plt.title("ROC-AUC Curve for Bagging GBDT Using Transformed Feature")
...
result_path = os.path.join(
    table_folder,
    "bagging_gbdt_transformed_feature_results.csv"
)
